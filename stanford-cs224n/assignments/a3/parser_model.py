...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        weights_hidden = torch.empty(self.n_features*self.embed_size, self.hidden_size)
        bias_hidden = torch.empty(self.hidden_size)
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(weights_hidden))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(bias_hidden))
        
        self.dropout = nn.Dropout(p=dropout_prob)

        weights_outer = torch.empty(self.hidden_size, self.n_classes)
        bias_outer = torch.empty(self.n_classes)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(weights_outer))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(bias_outer))
    ...
    def embedding_lookup(self, w):
        ...
        ### self.embeddings (num_words, embedding_size)
        x = (
          torch
            .index_select(self.embeddings, 0, w.flatten())
            .view(w.shape[0], self.n_features*self.embed_size)
        )
    ...
    def forward(self, w):
        ...
        h_out = torch.matmul(embeddings, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        h_act = torch.nn.functional.relu(h_out)
