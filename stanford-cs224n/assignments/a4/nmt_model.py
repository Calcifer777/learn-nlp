...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=embed_size, hidden_size=hidden_size, num_layers=1, dropout=self.dropout_rate, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens_0, (last_hidden, last_cell) = self.encoder(X_packed)
        enc_hiddens_1, _ = pad_packed_sequence(enc_hiddens_0)
        enc_hiddens = torch.permute(enc_hiddens_1, (1, 0, 2))
        # Decoder inputs
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, split_size_or_sections=1, dim=0):
          ...
          Ybar_t = torch.cat(tensors = (Y_t_squeezed, o_prev), dim=1)
